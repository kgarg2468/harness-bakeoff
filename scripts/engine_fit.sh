...
# Expected: our_version fits; pydantic-ai 2.31.1 fits; pydantic-ai >= 2.32 does not.
set -uo pipefail
ENGINE_REQS=(
...
status=0
check "our_version (httpx only)"                   fits     "httpx==0.28.1"                                   || status=1
check "pydantic_version @ 2.31.1 (openai 2.x)"     fits     "pydantic-ai-slim[openai,openrouter]==2.31.1"     || status=1
check "pydantic_version @ 2.32.0 (needs openai 3)" conflict "pydantic-ai-slim[openai,openrouter]==2.32.0"     || status=1
# Pin the exact latest release; an unpinned spec would just backtrack to 2.31.1.
latest="$(curl -fsSL https://pypi.org/pypi/pydantic-ai-slim/json | python3 -c 'import json,sys; print(json.load(sys.stdin)["info"]["version"])')"
check "pydantic_version @ latest ($latest)"        conflict "pydantic-ai-slim[openai,openrouter]==$latest"   || status=1
exit $status
