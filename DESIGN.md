...
data/rocketride/       catalog of RocketRide node definitions + example pipes (MIT, see notices)
...
4. on `turn.end` with stop `end_turn`, `max_steps`, `budget`, `cancelled` or `error`, commits the
   ...
   emits `commit`. A `paused` turn is not committed until the resumed turn finishes.
...
`MockEngine` is built from the real RocketRide node definitions (`data/rocketride/catalog.json`,
