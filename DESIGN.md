...
response's done event sent is 400. Its `expect` checks read `input` (e.g. `input_len`,
`last_type`, `tool_result_contains` by `call_id`, `reasoning_replayed`: the item exactly as
sent). Details: `fakeprov/README.md`.
